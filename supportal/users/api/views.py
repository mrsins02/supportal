...
from supportal.chat.api.serializers import (
    ChatDetailSerializer,
    ChatSerializer,
    NewChatSerializer,
)
from supportal.chat.models import Chat
from supportal.users.api.serializers import (
    LoginSerializer,
    ProfileSerializer,
    SendOTPSerializer,
)
...
class AuthViewSet(ViewSet):
    ...
    @extend_schema(
        methods=["POST"], request=SendOTPSerializer, responses=SendOTPSerializer
    )
    ...
    def sent_otp(self, request):
        ...
        if data_serializer.is_valid():
            ...
            return Response(
                data={"data": "an otp has been sent to given phone number"},
                status=status.HTTP_200_OK,
            )
        return Response(data=data_serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    @extend_schema(methods=["POST"], request=LoginSerializer, responses=LoginSerializer)
...
class ProfileViewSet(ListModelMixin, RetrieveModelMixin, GenericViewSet):
    ...
    @extend_schema(
        methods=["PUT", "PATCH"], request=ProfileSerializer, responses=ProfileSerializer
    )
    @action(
        methods=["PUT", "PATCH"],
        detail=False,
    ...
    def get_user_chat(self, request):
        ...
        if data_serializer.is_valid():
            ...
            chat = (
                Chat.objects.filter(members=request.user.pk)
                .filter(members=user.pk)
                .first()
            )
            if chat:
            ...
            return Response(
                data={"error": "chat not found"}, status=status.HTTP_404_NOT_FOUND
            )
        return Response(data=data_serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    @extend_schema(
        methods=["POST"], request=NewChatSerializer, responses=ChatSerializer
    )
    @action(methods=["POST"], detail=False, url_path="new-chat", url_name="new-chat")
    def new_chat(self, request):
        data_serializer = NewChatSerializer(
            data=request.data, context={"creator_user": request.user}
        )
        if data_serializer.is_valid():
